...
namespace FarmFuryStampede.EditorTools
{
    ...
    public static class StampedeUIArt
    {
        ...
        // The poster with its edges faded to transparent, generated by setup, so the slightly zoomed-out poster blends
        // into the plain farm scene (Canvas.png, the same art without the logo) drawn behind it.
        private const string LandingPosterSoftFile = "FF_StampedePoster_Soft.png";
        private const float PosterFeather = 0.06f;   // fade band, as a fraction of each dimension
        ...
        private static IEnumerable<string> MenuSpriteFiles()
        {
            ...
            yield return $"{UIDir}/{LandingPosterSoftFile}";
        ...
        public static Sprite LandingPoster() => Load(LandingPosterSoftFile);
        public static Sprite LandingBackdrop() => WorldSelectBackground();
        ...
        // Writes FF_StampedePoster_Soft.png: the poster with alpha ramping 0->1 (smoothstep) across the outer
        // PosterFeather of each edge. Rebuilt on every setup run so a replaced poster is picked up.
        private static void BuildSoftPoster()
        {
            string source = $"{UIDir}/{LandingPosterFile}";
            if (!File.Exists(source))
            {
                Debug.LogWarning($"[UIArt] Missing {source}; the landing screen shows a plain title.");
                return;
            }
            ...
            float bandX = Mathf.Max(1f, w * PosterFeather), bandY = Mathf.Max(1f, h * PosterFeather);
            for (int y = 0; y < h; y++)
            {
                float ay = Mathf.SmoothStep(0f, 1f, Mathf.Min(y, h - 1 - y) / bandY);
                for (int x = 0; x < w; x++)
                {
                    float ax = Mathf.SmoothStep(0f, 1f, Mathf.Min(x, w - 1 - x) / bandX);
                    int i = y * w + x;
                    pixels[i].a = (byte)Mathf.RoundToInt(pixels[i].a * ax * ay);
                }
            }
            texture.SetPixels32(pixels);
            File.WriteAllBytes($"{UIDir}/{LandingPosterSoftFile}", texture.EncodeToPNG());
            Object.DestroyImmediate(texture);
        }
