...
- **Real environment art** (Meadow Ruins only so far) spans three editor tools, all called from `StampedePhase5aSetup.RunSetup` so a normal re-run keeps everything in sync:
  ...
  - **Ground/platform tiles are procedural, not AI-generated** (`StampedeProceduralTiles.cs`): two rounds of AI art (Kling, then Leonardo) only ever produced isometric block icons despite explicit "side elevation, not isometric" prompts, so `GroundTile`/`PlatformTile` are generated directly - mottled dirt-and-stone (deliberately **no grass cap**: `GroundTile` is the single repeating Tile used for the whole ground mass, `GroundDepth` 6 tiles deep plus the tall level-boundary walls, so a cap would stripe down every buried/wall column; a true grass-topped-surface-only look needs Unity's Rule Tile autotiling, which this project doesn't use yet) and wood-plank art (safe for `PlatformTile` - floating platforms are always exactly one tile thick).
  - **Gameplay/UI art** (`StampedeUIArt.cs`, `Assets/_Project/Sprites/UI/`): `CropPickup` picks a random sprite from a 9-item normal pool and a 2-item secret-cluster pool instead of one placeholder icon; `Checkpoint` swaps real idle/activated signpost art (`CheckpointFlag.png` / `_pass.png`) instead of tinting a square; `Goal` uses `LevelComplete.png` (a checkered finish flag, despite the filename); `LevelBuilder` fills every `Chamber()` interior (Level 3 and Level 5's secrets) with `BarrierChamberWall.png`; the `BreakableFloor` tile uses `FloorTile.png` (`Sprites/Environment/`, cracked dirt with grass tufts - a better fit than the main ground texture since its jagged broken edges don't tile seamlessly, but read well as "about to give way"). `Collectable Coin.png` is left unused - it reads as Phase 6 coin-economy art, not a level crop.
